...
export default function QuestionStatCard({
  ...
  return (
    <div className="rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-[#181a1d] p-6 shadow-xs transition-shadow hover:shadow-sm">
      {/* Header */}
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="flex items-start gap-2.5">
          <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-gray-100 dark:bg-gray-800 text-xs font-semibold text-gray-500 dark:text-gray-400">
            {index + 1}
          </span>
          <div>
        ...
        {stat.question_type === "rating" && (
          <div className="flex flex-col sm:flex-row sm:items-center gap-6">
            <div className="flex items-center gap-3">
              <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-amber-50 dark:bg-amber-950/40 text-2xl font-bold text-amber-600 dark:text-amber-400">
            ...
            {stat.rating_distribution && (
              <div className="flex-1 space-y-1.5 border-t sm:border-t-0 sm:border-l border-gray-100 dark:border-gray-800 sm:pl-6 pt-3 sm:pt-0">
                {Object.entries(stat.rating_distribution).map(([ratingKey, count]) => {
                  ...
                  return (
                    <div key={ratingKey} className="flex items-center gap-2 text-xs">
                      <span className="w-5 text-right font-medium text-gray-500 dark:text-gray-400">
                        {ratingKey}★
                      </span>
